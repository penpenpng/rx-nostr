# Task 03: RelayCommunicationCollection、lease、hot relays
...
- normalized URL ごとに `RelayCommunication` を一つ作る per-instance collection を実装する。
...
- collection entry の eviction policy を決める。少なくとも active lease/query がある entry は削除しない。
...
- per-instance `RelayCommunicationCollection` が URL を再正規化し、一つの normalized URL に一つの `RelayCommunication` を割り当てる。
- 初期 v4 では collection entry を idle eviction しない。entry は RxNostr instance の dispose まで保持し、collection dispose で全 entry を一度だけ破棄する。
...
- query の `linger` は `ConnectionDemandScope` が lease の release を遅延させる。scope dispose は timer を取り消して即時 release し、collection/communication dispose 後の callback は transport を再操作しない。
