...
- The first connection demand checks the NIP-11 cache and, when it is empty, starts a fetch bounded by `nip11Timeout` (30 seconds by default). REQs wait for that attempt to settle. A failed attempt is recorded and then falls back to the newest cached metadata or an empty object; it does not fail the operation. `skipFetchNip11` skips the attempt but does not disable metadata already present in the configured directory, including `maxSubscriptions`.
