...
import {
  RelayProtocolSession,
  type RelayProtocolSessionOptions,
} from "./relay-protocol-session.ts";
import { RelayReqScheduler } from "./relay-req-scheduler.ts";
import { NostrTransport } from "./transport/index.ts";

function setup(options: RelayProtocolSessionOptions = {}) {
  ...
  const session = new RelayProtocolSession(url, transport, options);
