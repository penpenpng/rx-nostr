...
<p>rx-nostr の各所に存在する Observable は純粋な RxJS の Observable インスタンスなので、<a href="https://rxjs.dev/guide/operators" target="_blank" rel="noopener noreferrer">operator</a> を適用することができます。<code v-pre>RxReq</code> だけは Observable の他に REQ Strategy 等の情報を保持するためこれそのものは Observable インスタンスではないのですが、RxJS の <code v-pre>observable.pipe()</code> と完全な互換性を持つ <code v-pre>pipe()</code> メソッドを備えているので、やはり同様に operator を適用することができます。</p>
<p>rx-nostr が提供する operator はもちろんのこと、RxJS が提供する強力な operator の力を借りることも可能です。以下はその一例です。より多くの例は <RouteLink to="/v1/examples.html">Examples</RouteLink> を参照してください。</p>
