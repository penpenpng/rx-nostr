...
<p><code v-pre>createRxNostr()</code> の <code v-pre>authenticator</code> オプションを設定すると、rx-nostr は<a href="https://github.com/nostr-protocol/nips/blob/master/42.md" target="_blank" rel="noopener noreferrer">NIP-42</a> に基づく AUTH メッセージを自動でハンドリングするようになります。この機能が有効化されている場合、<code v-pre>auth-required</code> のステータスで拒否された REQ または EVENT は AUTH の成功後に自動で再送されます。</p>
