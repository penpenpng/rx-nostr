...
<div class="language-typescript" data-ext="ts" data-title="ts"><pre v-pre class="language-typescript"><code>rxNostr<span class="token punctuation">.</span><span class="token function">setDefaultRelays</span><span class="token punctuation">(</span><span class="token keyword">await</span> window<span class="token punctuation">.</span>nostr<span class="token punctuation">.</span><span class="token function">getRelays</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
<p><code v-pre>rxNostr.send()</code> や <code v-pre>rxNostr.use()</code> などの第二引数に <code v-pre>relays</code> オプションを渡すことによって、<strong>一時リレー</strong> の上で通信することができます。一時リレーは <RouteLink to="/v2/connection-strategy.html">Connection Strategy</RouteLink> の設定に関わらず、必要な間だけ接続され不要になると切断されます。</p>
