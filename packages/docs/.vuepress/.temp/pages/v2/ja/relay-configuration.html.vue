...
<p><code v-pre>rxNostr.send()</code> や <code v-pre>rxNostr.use()</code> などの第二引数に <code v-pre>relays</code> オプションを渡すことによって、<strong>一時リレー</strong> の上で通信することができます。一時リレーは <RouteLink to="/v2/ja/connection-strategy.html">Connection Strategy</RouteLink> の設定に関わらず、必要な間だけ接続され不要になると切断されます。</p>
