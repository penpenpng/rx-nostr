<template><div><h1 id="req-strategy" tabindex="-1"><a class="header-anchor" href="#req-strategy" aria-hidden="true">#</a> REQ Strategy</h1>
...
<h2 id="oneshot-strategy" tabindex="-1"><a class="header-anchor" href="#oneshot-strategy" aria-hidden="true">#</a> Oneshot Strategy</h2>
<p>Oneshot Strategy は Backward Strategy と同じく既に発行された過去のイベントを取得するための戦略ですが、<code v-pre>ReqPacket</code> をひとつしか送出することができません。その代わり、最初の REQ サブスクリプションが CLOSE したとき Observable が complete します。 <code v-pre>rxNostr.use(rxReq)</code> が返す <code v-pre>Observable&lt;EventPacket&gt;</code> が (<code v-pre>rxNostr.dispose()</code> が呼ばれる以外の理由で) complete する戦略はこれだけです。この戦略は必要なデータの読み込みが完了したときに何かを実行したい場合に便利です。</p>
