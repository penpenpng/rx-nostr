...
<p>rx-nostr の中心となるのは <RouterLink to="/guide/api/rx-nostr.html"><code v-pre>RxNostr</code></RouterLink> と <RouterLink to="/guide/api/rx-req.html"><code v-pre>RxReq</code></RouterLink> の 2 種類のオブジェクトです。<code v-pre>RxNostr</code> は <code v-pre>RxReq</code> が提供する <a href="https://rxjs.dev/guide/observable" target="_blank" rel="noopener noreferrer">Observable<ExternalLinkIcon/></a> を <code v-pre>subscribe()</code> し、あなたのプログラムは <code v-pre>RxNostr</code> が提供する Observable を <code v-pre>subscribe()</code> します。これらの Observable の中を流れるオブジェクトを Packet と呼びます。この関係は概ね次の図で表現できます:</p>
<p><img src="@source/guide/data-flow.png" alt="flow"></p>
<p>この関係は <code v-pre>RxNostr</code> の <code v-pre>use()</code> メソッドによって確立され、その戻り値を <code v-pre>subscribe()</code> することで初めて駆動し、さらにそれを <code v-pre>unsubscribe()</code> することで解消されます。</p>
